...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    set -x
    cryptogen generate \
    --config=./organizations/cryptogen/crypto-config-hospital-admin.yaml \
    --output="organizations"
    res=$?
    { set +x; } 2>/dev/null

    if [ $res -ne 0 ]; then
      fatalln "Failed to generate Hospital Admin certificates..."
    fi
    ...
    # Clinical Staff
    # =====================================================
    infoln "Creating Clinical Staff Identities"

    set -x
    cryptogen generate \
    --config=./organizations/cryptogen/crypto-config-clinical-staff.yaml \
    --output="organizations"
    res=$?
    { set +x; } 2>/dev/null

    if [ $res -ne 0 ]; then
      fatalln "Failed to generate Clinical Staff certificates..."
    fi
    ...
    # Diagnostic Staff
    # =====================================================
    infoln "Creating Diagnostic Staff Identities"

    set -x
    cryptogen generate \
    --config=./organizations/cryptogen/crypto-config-diagnostic-staff.yaml \
    --output="organizations"
    res=$?
    { set +x; } 2>/dev/null

    if [ $res -ne 0 ]; then
      fatalln "Failed to generate Diagnostic Staff certificates..."
    fi
    ...
    # Pharmacy Services
    # =====================================================
    infoln "Creating Pharmacy Services Identities"

    set -x
    cryptogen generate \
    --config=./organizations/cryptogen/crypto-config-pharmacy-services.yaml \
    --output="organizations"
    res=$?
    { set +x; } 2>/dev/null

    if [ $res -ne 0 ]; then
      fatalln "Failed to generate Pharmacy Services certificates..."
    fi
    ...
    set -x
    cryptogen generate \
    --config=./organizations/cryptogen/crypto-config-insurance-provider.yaml \
    --output="organizations"
    res=$?
    { set +x; } 2>/dev/null

    if [ $res -ne 0 ]; then
      fatalln "Failed to generate Insurance Provider certificates..."
    fi
    ...
    # Patient Access
    # =====================================================
    infoln "Creating Patient Access Identities"

    set -x
    cryptogen generate \
    --config=./organizations/cryptogen/crypto-config-patient-access.yaml \
    --output="organizations"
    res=$?
    { set +x; } 2>/dev/null

    if [ $res -ne 0 ]; then
      fatalln "Failed to generate Patient Access certificates..."
    fi
  ...
  if [ "$CRYPTO" == "cfssl" ]; then
    ...
    # Hospital Admin
    peer_cert peer peer0.hospitaladmin.example.com hospitaladmin
    peer_cert admin Admin@hospitaladmin.example.com hospitaladmin

    # Clinical Staff
    peer_cert peer peer0.clinicalstaff.example.com clinicalstaff
    peer_cert admin Admin@clinicalstaff.example.com clinicalstaff

    # Diagnostic Staff
    peer_cert peer peer0.diagnosticstaff.example.com diagnosticstaff
    peer_cert admin Admin@diagnosticstaff.example.com diagnosticstaff

    # Pharmacy Services
    peer_cert peer peer0.pharmacy.example.com pharmacy
    peer_cert admin Admin@pharmacy.example.com pharmacy

    # Insurance Provider
    peer_cert peer peer0.insurance.example.com insurance
    peer_cert admin Admin@insurance.example.com insurance

    # Patient Access
    peer_cert peer peer0.patientaccess.example.com patientaccess
    peer_cert admin Admin@patientaccess.example.com patientaccess
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    infoln "Creating Diagnostic Staff Identities"
    createDiagnosticStaff

    infoln "Creating Pharmacy Services Identities"
    createPharmacyServices
