...
export class Proxy {
	...
	async getPeersStatus(network_id, channel_genesis_hash) {
		const client = await this.platform.getClient(network_id);
		const channel_name =
			client.getChannelNameByHash(channel_genesis_hash) || channel_genesis_hash;
		const nodes = await this.persistence
			.getMetricService()
			.getPeerList(network_id, channel_genesis_hash);
		const clientReady = client && client.getStatus && client.getStatus();
		let discover_results;
		let orderersList = [];
		if (clientReady && client.fabricGateway && channel_name) {
			try {
				orderersList = await client.fabricGateway.getActiveOrderersList(
					channel_name
				);
		...
		for (const node of nodes) {
			...
			if (node.peer_type === 'PEER') {
				...
				node.ledger_height_high = '-';
				node.ledger_height_unsigned = '-';
				const orderersCount = orderersList.length;
				if (orderersCount !== 0) {
	...
	getLatestBlockTime(channel) {
		...
		const latestBlockEntry = channel.latestdate;
	...
	async getChannelsInfo(network_id) {
		try {
			...
			if (
				...
				!client.getStatus ||
				!client.getStatus() ||
				!client.fabricGateway
			) {
				logger.warn(
					`Client peer is unavailable for network ${network_id}; returning DB channel data only`
				);
				return channels;
			}
			...
			for (const channel of channels) {
				...
				const agoBlockTimes = this.getLatestBlockTime(channel);
				const channel_members = await client.fabricGateway.queryEndorsersCommitter(
					channel.channelname
				);
	...
	async queryTxFromLedger(
		...
		const syncPlatform = new SyncPlatform(this.persistence, null);
		const sync = new SyncServices(syncPlatform, this.persistence);
		const client = this.platform.getClient(network_id);
