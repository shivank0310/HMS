...
export class CRUDService {
	...
	async getTxList(
       ...
       inner join channel on t.channel_genesis_hash=channel.channel_genesis_hash and t.network_name = channel.network_name where  t.blockid >= $1 and t.id >= $2 and
		...
		const txnsData = await this.sql.getRowsBySQlQuery(sqlTxList, values);
		const response = {
			txnsData: txnsData,
	...
	async getBlockAndTxList(
		...
		const blocksData = await this.sql.getRowsBySQlQuery(sqlBlockTxList, values);
		const noOfpages = Math.ceil(countOfBlocks / size);
		const response = {
