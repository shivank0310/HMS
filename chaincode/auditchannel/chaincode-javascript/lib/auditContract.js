...
class AuditContract extends Contract {
    constructor() {
        super('AuditContract');
        this.channelName = 'auditchannel';
        this.recordType = 'AuditRecord';
    }
    ...
    _applyBusinessRules(record) {
        ...
        if (normalized.timestamp !== undefined && !isValidDate(normalized.timestamp)) {
        ...
        if (normalized.actor !== undefined) {
            normalized.actor = String(normalized.actor).trim();
        }
        ...
        if (normalized.resourceType !== undefined) {
            normalized.resourceType = String(normalized.resourceType).trim();
        }
    ...
    async _createRecord(ctx, recordId, fieldsJson) {
        ...
        const record = this._applyBusinessRules({
            ...
            timestamp: dynamicFields.timestamp || timestamp,
            createdAt: timestamp,
            updatedAt: timestamp,
        });
    ...
    async _updateRecord(ctx, recordId, fieldsJson) {
        ...
        const updated = this._applyBusinessRules({
            ...
            ...dynamicFields,
            recordId,
            auditId: existing.auditId || recordId,
            docType: this.recordType,
            channel: this.channelName,
            timestamp: dynamicFields.timestamp || existing.timestamp || timestamp,
    ...
    async LogAudit(ctx, recordId, fieldsJson) {
        return this._createRecord(ctx, recordId, fieldsJson);
    }

    async UpdateAudit(ctx, recordId, fieldsJson) {
        return this._updateRecord(ctx, recordId, fieldsJson);
    }

    async GetAudit(ctx, recordId) {
        return this._getRecord(ctx, recordId);
    }

    async DeleteAudit(ctx, recordId) {
    ...
    async QueryAuditByField(ctx, fieldName, fieldValue) {
        return this._queryByField(ctx, fieldName, fieldValue);
    }

    async InitLedger(ctx) {
        const sampleId = 'audit-sample-001';
        ...
        await this._createRecord(
            ...
            JSON.stringify({
                ...
                severity: 'info',
                status: 'recorded',
                note: 'Bootstrap audit record for the audit channel',
            })
        );
        return sampleId;
    }
